...
class InterpolateForm(Form):
    ...
    mode = TypedChoiceField(choices=MODE_TYPES, required=True, initial=MODE_TYPES[0],
                            label='Select Mode', widget=Select(attrs={'class': 'form-select'}))
    ...
    redshift = FloatField(required=True, label='Cosmic Redshift', widget=NumberInput(attrs={'class': 'form-control'}))
